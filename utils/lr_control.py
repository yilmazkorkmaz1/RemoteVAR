...
def lr_wd_annealing(
    ...
    *,
    ...
    lr_floor = float(min_lr) if min_lr is not None else 0.0
    if lr_floor < 0:
        raise ValueError(f"min_lr must be >= 0, got {lr_floor}")
    wp_it = round(wp_it)
    if cur_it < wp_it:
    ...
    inf = 1e6
    min_lr_out, max_lr_out = inf, -1
    min_wd, max_wd = inf, -1
    for param_group in optimizer.param_groups:
        lr = cur_lr * param_group.get('lr_sc', 1)    # 'lr_sc' could be assigned
        if lr_floor > 0:
            lr = max(lr, lr_floor)
        param_group['lr'] = lr
        max_lr_out = max(max_lr_out, lr)
        min_lr_out = min(min_lr_out, lr)
    ...
    if min_lr_out == inf: min_lr_out = -1
    if min_wd == inf: min_wd = -1
    return min_lr_out, max_lr_out, min_wd, max_wd


def filter_params(model, nowd_keys=(), allow_frozen: bool = False) -> Tuple[
    ...
    # By default we enforce that all parameters are trainable. Some fine-tuning modes
    # intentionally freeze large parts of the model, so callers can set allow_frozen=True.
    if not allow_frozen:
