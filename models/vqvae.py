...
class VQVAE(nn.Module):
    ...
    def idxBl_to_h(self, gt_ms_idx_Bl: List[torch.Tensor], *, include_next_scale: bool = False):
        return self.quantize.idxBl_to_var_input(gt_ms_idx_Bl, include_next_scale=include_next_scale)
    ...
    def fhat_to_img(self, f_hat: torch.Tensor, *, decoder_skips: Optional[Sequence[torch.Tensor]] = None):
        return self.decoder(self.post_quant_conv(f_hat), skips=decoder_skips).clamp_(-1, 1)
    ...
    def embed_to_img(
        ...
        last_one=False,
        *,
        decoder_skips: Optional[Sequence[torch.Tensor]] = None,
        ...
        if last_one:
            ...
            return [
                self.decoder(self.post_quant_conv(f_hat), skips=decoder_skips).clamp_(-1, 1)
                for f_hat in self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=all_to_max_scale, last_one=False)
            ]
    

    def encoder_context(self, images: List[torch.Tensor]):
        pre_image = images[0]
        post_image = images[1]
        pre_context = self.encoder.forward_context(pre_image)
        post_context = self.encoder.forward_context(post_image)

        context = []
        for context1, context2 in zip(pre_context, post_context):
            context.append(context2 - context1)
        return context
