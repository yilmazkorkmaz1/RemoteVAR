...
class CrossAttention(nn.Module):
    def __init__(
        self, block_idx, context_dim, embed_dim=768, num_heads=12,
        attn_drop=0., proj_drop=0., attn_l2_norm=False, flash_if_available=True,
        context_len=None, inner_dim=1024,
    ):
        super().__init__()
        assert embed_dim % num_heads == 0
        # IMPORTANT: inner_dim must be divisible by num_heads, otherwise q/k/v reshape will break.
        # This can happen for depth=12 (num_heads=12) with the default inner_dim=1024.
        inner_dim = int(inner_dim)
        num_heads = int(num_heads)
        if inner_dim % num_heads != 0:
            # Pick the nearest smaller divisible dim to preserve compute/memory.
            adj = (inner_dim // num_heads) * num_heads
            if adj <= 0:
                adj = num_heads
            inner_dim = adj
        self.inner_dim = inner_dim

        self.block_idx, self.num_heads, self.head_dim = block_idx, num_heads, self.inner_dim // num_heads  # =64
        ...
        self.mat_q = nn.Linear(embed_dim, self.inner_dim, bias=False)
        self.mat_kv = nn.Linear(context_dim, self.inner_dim * 2, bias=False)

        self.q_bias, self.v_bias = nn.Parameter(torch.zeros(self.inner_dim)), nn.Parameter(torch.zeros(self.inner_dim))
        self.register_buffer('zero_k_bias', torch.zeros(self.inner_dim))
        
        self.proj = nn.Linear(self.inner_dim, embed_dim)
        self.proj_drop = nn.Dropout(proj_drop, inplace=True) if proj_drop > 0 else nn.Identity()
        self.attn_drop: float = attn_drop
        self.using_flash = flash_if_available and flash_attn_func is not None
        self.using_xform = flash_if_available and memory_efficient_attention is not None
        
        # Create 2D positional embeddings during init and register as buffer
        if context_len is None:
            raise ValueError("CrossAttention requires context_len at init (no lazy creators in forward).")
        h = w = int(math.sqrt(context_len))
        if h * w != int(context_len):
            raise ValueError(f"CrossAttention expects square context_len, got {context_len}.")
        pos_embed = self.get_2d_sincos_pos_embed(context_dim, h, w).unsqueeze(0)  # [1, h*w, context_dim]
    ...
    def get_2d_sincos_pos_embed(self, embed_dim, h, w, device=None, dtype=None):
        """
        ...
        pos_embed = self.get_1d_sincos_pos_embed_from_grid(embed_dim, grid)  # [h*w, embed_dim]
        return pos_embed
    ...
    def get_1d_sincos_pos_embed_from_grid(self, embed_dim, pos):
        ...
        Args:
            ...
            pos: a list of positions to be encoded: size (M, N)
        Returns:
        ...
        # pos: [M, N], omega: [embed_dim/2]
        out = torch.einsum('mn,d->nmd', pos, omega)  # [N, M, embed_dim/2]
        
        emb_sin = torch.sin(out)  # [N, M, embed_dim/2]
        emb_cos = torch.cos(out)  # [N, M, embed_dim/2]
        
        emb = torch.cat([emb_sin, emb_cos], dim=-1)  # [N, M, embed_dim]
        emb = emb.reshape(emb.shape[0], -1)  # [N, M*embed_dim]
        
        # Take only the first embed_dim dimensions
    ...
    def forward(self, x, context, attn_bias=None):
        ...
        kv = F.linear(input=context, weight=self.mat_kv.weight, bias=torch.cat((self.zero_k_bias, self.v_bias))).view(B, L_context, 2, self.num_heads, self.head_dim)
        ...
        if using_flash or self.using_xform:
            q = q.squeeze(2)  # Remove the extra dimension: [B, L, 1, H, c] -> [B, L, H, c]
            k, v = kv.unbind(dim=2)
            dim_cat = 1
        else: 
            q, = q.permute(2, 0, 3, 1, 4).unbind(dim=0)
        ...
        if self.caching:
            if self.cached_k is None:
                ...
                # Robustness: if caching leaks across calls with different batch shapes (e.g. after an exception),
                # reset instead of crashing on cat.
                if (
                    self.cached_k.shape[0] != k.shape[0]  # batch
                    or self.cached_k.shape[-1] != k.shape[-1]  # head dim
                    or self.cached_k.dtype != k.dtype
                    or self.cached_k.device != k.device
                ):
                    self.cached_k = k
                    self.cached_v = v
                else:
...
class AdaLNSABlock(nn.Module):
    def __init__(
        ...
        num_heads, context_dim=None, context_len=None, mlp_ratio=4., drop=0., attn_drop=0., drop_path=0., tau=4, cos_attn=False,
        flash_if_available=False, fused_if_available=True, disable_cross_attention=False, cross_attn_inner_dim=1024,
    ):
        ...
        # Only create cross-attention if not disabled
        if not disable_cross_attention:
            self.cross_attn = CrossAttention(block_idx=block_idx, embed_dim=embed_dim, context_dim=context_dim, num_heads=num_heads, attn_drop=attn_drop, proj_drop=drop, attn_l2_norm=True, flash_if_available=flash_if_available, context_len=context_len, inner_dim=cross_attn_inner_dim)
        else:
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(self, x, cond_BD, attn_bias, context):   # C: embed_dim, D: cond_dim
        if self.shared_aln:
            gamma1, gamma2, scale1, scale2, shift1, shift2 = (self.ada_gss + cond_BD).unbind(2) # 116C + B16C =unbind(2)=> 6 B1C
        else:
            gamma1, gamma2, scale1, scale2, shift1, shift2 = self.ada_lin(cond_BD).view(-1, 1, 6, self.C).unbind(2)

        # Only apply cross-attention if not disabled
        if not self.disable_cross_attention and self.cross_attn is not None:
            x = x + self.drop_path(self.cross_attn(self.ln_wo_grad(x), context, attn_bias=None))
    ...
    def extra_repr(self) -> str:
        return f'shared_aln={self.shared_aln}, disable_cross_attention={self.disable_cross_attention}'
