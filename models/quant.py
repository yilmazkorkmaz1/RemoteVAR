...
class VectorQuantizer2(nn.Module):
    ...
    def idxBl_to_var_input(
        ...
        *,
        ...
        Notes:
        - Default behavior (include_next_scale=False):
            For each stage s>0, return the cumulative f_hat built from GT tokens up to stage (s-1),
            downsampled to the stage's pn x pn and flattened to (B, pn^2, Cvae).
            This matches autoregressive generation where stage-s tokens are not available yet.

        - Current-scale token conditioning (include_next_scale=True):
            For each returned stage-s input, additionally include the *stage-s* GT residual contribution
            by temporarily adding the next-scale (stage-s) embedding into the cumulative f_hat *before*
            downsampling to pn x pn. This is intended ONLY for known conditioning streams (e.g., pre/post),
            not for predicted streams (e.g., mask), to avoid information leakage.
        """
        next_scales = []
        ...
        with torch.autocast(device_type=gt_ms_idx_Bl[0].device.type, enabled=False):
            ...
            pn_cur: int = int(self.v_patch_nums[0])
            for si in range(SN - 1):
                if self.prog_si == 0 or (0 <= self.prog_si-1 < si): break   # progressive training: not supported yet, prog_si always -1

                # Add current-scale residual contribution into the cumulative f_hat (GT for scale `si`).
                h_cur = self.embedding(gt_ms_idx_Bl[si]).transpose(1, 2).view(B, C, pn_cur, pn_cur)
                h_cur_up = F.interpolate(h_cur, size=(H, W), mode="bicubic")
                f_hat.add_(self.quant_resi[si / (SN - 1)](h_cur_up))

                pn_next = int(self.v_patch_nums[si + 1])

                # Teacher-forcing input for next stage: by default, use f_hat up to (si).
                # Optionally, also add the next-scale GT residual *temporarily* (for conditioning streams).
                f_for_out = f_hat
                if include_next_scale:
                    h_next = self.embedding(gt_ms_idx_Bl[si + 1]).transpose(1, 2).view(B, C, pn_next, pn_next)
                    h_next_up = F.interpolate(h_next, size=(H, W), mode="bicubic")
                    f_for_out = f_for_out + self.quant_resi[(si + 1) / (SN - 1)](h_next_up)

                next_scales.append(
                    F.interpolate(f_for_out, size=(pn_next, pn_next), mode="area")
                    .view(B, C, -1)
                    .transpose(1, 2)
                )
                pn_cur = pn_next
